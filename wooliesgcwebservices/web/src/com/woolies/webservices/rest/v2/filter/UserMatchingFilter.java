...
public class UserMatchingFilter extends AbstractUrlMatchingFilter
{
			...
			final FilterChain filterChain) throws ServletException, IOException
	{
		...
		if (auth != null)
		{
			...
			{
				setUserOrThrowError(auth, userID);
			}
			else
			{
	...
	/**
	 * This is used to set user or throw access denied error
	 *
	 * @param auth
	 * @param userID
	 */
	private void setUserOrThrowError(final Authentication auth, final String userID)
	{
	...
	/**
	 * This is used to set current user
	 *
	 * @param auth
	 */
	private void setCurrentuser(final Authentication auth)
	{
		if (hasRole(ROLE_CUSTOMERGROUP, auth) || hasRole(ROLE_CUSTOMERMANAGERGROUP, auth))
		{
			setCurrentUser((String) auth.getPrincipal());
