...
public class AddressDataErrorsPopulator implements Populator<AddressVerificationResult<AddressVerificationDecision>, Errors>
{
	...
	/**
	 * This method is used to populate Errors for AddressData
	 * 
	 * @param source
	 * @param target
	...
	{ //NOSONAR
		...
		for (final Map.Entry<String, AddressVerificationErrorField> entry : source.getErrors().entrySet())
		{
			target.rejectValue(
					attributeMappingMap.containsKey(entry.getKey()) ? attributeMappingMap.get(entry.getKey()) : entry.getKey(),
					entry.getValue().isInvalid() ? "field.invalid" : "field.required", new Object[]
					{ entry.getKey() }, "");
		}
	}
}
