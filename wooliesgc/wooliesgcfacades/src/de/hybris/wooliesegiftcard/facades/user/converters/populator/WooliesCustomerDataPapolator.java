...
/**
 * @author 648156 This class is used to populate CustomerData
 */
public class WooliesCustomerDataPapolator implements Populator<UserModel, CustomerData>
{
	...
	public void populate(final UserModel source, final CustomerData target) throws ConversionException
	{
		...
		{
			final CorporateB2BCustomerModel corporateb2bCustomer = (CorporateB2BCustomerModel) source;
			...
			if (!b2bPermissionModel.isEmpty())
			{
				...
				{
					thresholdValue = getThresholdValue(corporateb2bCustomer, thresholdValue, b2bPermission);
				}
			}

			target.setName(corporateb2bCustomer.getName());
			target.setUid(corporateb2bCustomer.getUid());
			target.setPhone(corporateb2bCustomer.getPhone());
			...
			target.setEmail(corporateb2bCustomer.getUserEmail());
			target.setCustomerId(corporateb2bCustomer.getCustomerID());
		}
	}
